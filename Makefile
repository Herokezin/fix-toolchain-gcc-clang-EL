...
ARCH		?=arm64
CROSS_COMPILE   ?= $(srctree)/NEMESIS-toolchain/gcc/linux-x86/aarch64/aarch64-linux-android-4.9/bin/aarch64-linux-android-
...
AS		= $(CROSS_COMPILE)as
LD		= $(CROSS_COMPILE)ld
CC              = $(srctree)/NEMESIS-toolchain/clang/host/linux-x86/clang-r416183d/bin/clang
CPP		= $(CC) -E
AR		= $(CROSS_COMPILE)ar
NM		= $(CROSS_COMPILE)nm
...
ifeq ($(cc-name),clang)
ifneq ($(CROSS_COMPILE),)
CLANG_TRIPLE    ?= $(srctree)/NEMESIS-toolchain/clang/host/linux-x86/clang-r416183d/bin/aarch64-linux-gnu-
